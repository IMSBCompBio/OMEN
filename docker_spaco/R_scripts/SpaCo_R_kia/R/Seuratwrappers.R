...
seurat_to_spaco <- function(Seurat, assay = "SCT", n_image = 1, slot = "scale.data") {
  ...
  tryCatch(
    {
      # Try the v4‐style coordinates slot
    ...
    error = function(e) {
      # If that fails (e.g. VisiumV2 with no @coordinates),
      # fall back to meta.data
      tissue_positions_list <<- Seurat@meta.data
      tissue_positions_list <<- tissue_positions_list[tissue_positions_list$in_tissue == 1, c("row", "col")]
    }
  )

  # hopefully we get a distance matrix
