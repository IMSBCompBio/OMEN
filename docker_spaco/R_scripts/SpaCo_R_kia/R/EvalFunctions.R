...
SVGTest <- function(SpaCoObject, adjustMethod = "holm") {
  ...
  if (inherits(GraphLaplacian, "dgCMatrix"))
  {
  ...
  if (!is.null(SpaCoObject@meta.data) && !is.na(ncol(SpaCoObject@meta.data)) && ncol(SpaCoObject@meta.data) > 0) {
  ...
  print("Here are the sigma eigenvalues\n\n")
  print(sigmaSVD)
  ...
  return(list(resDf = resDf, sigmaSVD = sigmaSVD))
}
